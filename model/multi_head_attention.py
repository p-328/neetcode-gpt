...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.output_gen = nn.Linear(in_features=attention_dim, out_features=attention_dim, bias=False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        scores = [head(embedded) for head in self.heads]
        concat_scores = torch.cat(scores, dim=2)
        return torch.round(self.output_gen(concat_scores), decimals=4)
